...
pub async fn start_server() {
    ...
    tokio::spawn({
        ...
        async move { process_updates(config.tg.bot_id, &tg_client, &redis_service).await }
    });
...
async fn process_updates(bot_id: i64, tg_client: &TgClient, redis_service: &RedisService) {
    while let Some(upd) = tg_client.get_updates().await.next().await {
        println!("got update {:#?}", upd);
        // redis_service.save_message(bot_id, upd);
